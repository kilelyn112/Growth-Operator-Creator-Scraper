...
export default function ResultsTable({ creators, summary, jobId, platform = 'youtube' }: ResultsTableProps) {
  ...
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--bg-border)] rounded-xl overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-[var(--bg-border)] flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-8">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-2xl font-mono font-bold text-[var(--signal-success)]">
              {summary.qualified}
            </span>
          </div>
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <span className="text-2xl font-mono font-bold text-[var(--accent-gold)]">
                {summary.withEmail}
              </span>
              {summary.withEmail > 0 && (
                <span className="text-xs px-2 py-0.5 rounded bg-[rgba(251,191,36,0.15)] text-[var(--accent-gold)] font-mono">
                  TARGETS
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <div
              className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${
                filterQualified
                  ? 'bg-[var(--signal-success)] border-[var(--signal-success)]'
          ...
          <a
            ...
            download
            className="group flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[var(--signal-action)] text-[var(--bg-deep)] font-semibold text-sm hover:brightness-110 transition-all duration-150 glow-action"
          >
          ...
          <thead>
            <tr className="bg-[var(--bg-elevated)]">
              ...
              <th
                ...
                className="px-6 py-3 text-center text-xs font-mono font-medium text-[var(--text-muted)] uppercase tracking-wider cursor-pointer hover:text-[var(--text-secondary)] transition-colors"
              >
              ...
              sortedCreators.map((creator, index) => {
                ...
                return (
                  <tr
                    ...
                    className={`animate-row transition-all duration-150 ${
                      gold
                        ? 'bg-[rgba(251,191,36,0.05)] hover:bg-[rgba(251,191,36,0.1)]'
                        : creator.qualified
                        ? 'bg-[rgba(34,197,94,0.03)] hover:bg-[var(--bg-elevated)]'
                  ...
                  >
                      ...
                      {creator.email ? (
                        <a
                          ...
                          className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg transition-all duration-150 ${
                            gold
                              ? 'bg-[var(--signal-action-dim)] text-[var(--signal-action)] hover:bg-[var(--signal-action)] hover:text-[var(--bg-deep)]'
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      {creator.qualified ? (
                        ...
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-mono font-medium bg-[rgba(239,68,68,0.15)] text-[var(--signal-danger)]">
                          <span className="w-1.5 h-1.5 rounded-full bg-[var(--signal-danger)]"></span>
                          REJECTED
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 max-w-xs">
                      <span
                        ...
                        title={creator.qualificationReason}
                      >
      ...
      {/* Footer */}
      <div className="px-6 py-3 border-t border-[var(--bg-border)] bg-[var(--bg-base)] flex items-center justify-between">
        ...
        {summary.withEmail > 0 && (
          <span className="text-sm font-mono text-[var(--accent-gold)]">
            {summary.withEmail} ready for outreach
          </span>
        )}
      </div>
    </div>
